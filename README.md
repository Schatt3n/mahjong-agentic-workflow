...
- 双层进展保护：私聊主循环第二步起向模型注入三行内执行摘要，允许模型自报停滞并提前求助；后端继续检测重复观察、短周期循环和连续无进展作为兜底。
...
handle user message
  ...
  -> inspect optional model self-assessment
  -> if tool calls:
...
结构化观测记录可通过 `GET /api/channels/wechaty/observations` 查询，并支持 `room_id`、`room_topic`、`semantic_action` 和 `limit` 参数。
...
- 按 `conversation_id` 隔离的聊天室时间线，以及每位模拟用户的原始输入和 Agent 最终回复；每轮会标记“业务对话 / 闲聊插入 / 回到业务”；
- GLM 生成来源、模型、trace、耗时与失败回退原因；
- DeepSeek 主链路 trace、目标状态、工具调用、HTTP 状态与耗时；
- 启动、立即运行、暂停、恢复等固定白名单控制，页面不能执行任意命令。

多轮对话采用真实响应驱动的闭环，而不是预先生成固定脚本：调度器发送一条虚拟用户消息后，必须等待主 Agent 返回 `final_reply`，再将回复写入该用户独立的 `DialogState`。`BehaviorPolicy.following_action()` 根据角色画像、最后一条 Agent 回复、业务锚点和当前阶段选择回答追问、插入闲聊、返回原业务或自然沉默；启用 GLM 时，模型只负责把这一确定意图改写成自然用户话术。闲聊与业务恢复继续使用原 `conversation_id`，因此主 Agent 能看到同一会话历史。重复回复触发无进展停止，24 轮仅是异常安全上限，不是固定业务轮数。
...
仓库的规范性设计文档是 [Mahjong Ops Agent 设计准则](docs/design_principles.md)；所有编码 Agent 在修改代码前必须阅读并遵守根目录 [AGENTS.md](AGENTS.md)。核心要求如下：

1. 主 Agent 负责目标理解、任务规划、工具选择和动态调整，后端不维护不断增长的自然语言业务决策树。
2. 模型只能提出动作，不能绕过 Tool Gateway、事务、权限、状态机或幂等边界修改状态。
3. 工具结果必须回喂模型；客户回复不能虚构未执行动作，也不能把内部工具、审批和候选人搜索细节暴露给客户。
4. 数据库是业务事实源；conversation、task context、game、customer 和 trace 标识不能混用。
5. 所有副作用必须可校验、可去重、可恢复、可审计；客户可见文本必须依次经过最小数据投影、模型生成/审查和确定性关键词/模式过滤，模型审查不能作为唯一防线。
