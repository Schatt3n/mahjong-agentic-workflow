...
旧 trial/workflow 代码和历史 v2 链路仅作为 legacy/reference 保留，不接入当前主服务、上下文构建、工具网关或默认评测入口。当前稳定 import 面、主实现包和默认启动入口都收敛为 `mahjong_agent_runtime`。
...
- `record_badcase` 必须包含 `reason / input / actual / expected`，保证记录的是可评测样本，而不是不可复盘的普通日志。
...
- `ContextBuilder` 每轮只负责打包上下文，不解释麻将语义，不决定下一步动作。
- 每轮上下文包含当前消息、近期对话、客户画像、当前局池、待审批草稿、可用工具、上一轮工具结果、输出合同和 `conversation_checkpoint`。
- `ContextPackingPolicy` 会按 token 预算裁剪近期对话，并把裁剪数量、是否带 checkpoint、checkpoint 来源 trace 写入 `context_packed` trace。
...
- `tests/test_agent_runtime.py`：验证模型驱动工具顺序、工具错误回喂模型、后端不解释短确认语义、上下文 checkpoint、预算拒绝、消息幂等、并发去重、JSONL trace 可回放、SQLite 状态可恢复。
- `scripts/run_agent_runtime_eval.py`：当前主链路 regression eval，使用稳定 `mahjong_agent_runtime` import 面。
- `scripts/run_evals.py`：当前主链路默认评测，只运行当前 Agent Runtime 边界、runtime eval 和主链路 pytest；显式加 `--live-real-owner` 时，会追加真实老板聊天 live LLM eval。
- `scripts/run_legacy_evals.py`：旧 V2/trial/workflow 参考回归，保留用于对照，不代表当前主链路。

## 持久化

- 本地主服务默认使用 SQLite：`data/agent_runtime.sqlite3`。
- SQLite 持久化客户、局、邀约草稿、通用外发消息草稿、对话 turn、上下文 checkpoint、工具幂等 claim/result、消息幂等结果、badcase、状态变化。
- Trace 使用 JSONL：`logs/agent_runtime_trace.log`，可按 `traceId` 结构化回放模型输入、模型输出、工具调用、工具结果和状态变化。
...
- 人工 badcase 不直接写库，会先构造 `record_badcase` 工具调用，再经过 `ToolGateway` 的 schema、权限、幂等和 trace。
