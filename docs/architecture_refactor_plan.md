...
- `tool_permissions.py` 已新增共享“最终动作 -> 可用工具”白名单：`ActionValidator` 生成 `ValidatedAction` 时会先复验 `required_tools`，`ToolOrchestrator` 执行前也会再次校验。即使上游 `required_tools` 被污染，不属于当前 `effective_action` 的工具也不会进入执行，例如最终动作只是 `ask_create_confirmation` 时，`create_game/create_pending_outbox/search_candidate_customers` 都不能执行；这层只做权限校验，不重新判断用户语义。
...
- `SemanticResolver` 已开始执行语义 contract 验收：`intent/proposed_action/confidence/reasoning_summary/slots` 是必需字段，`slots/action_arguments/profile_observations` 必须是约定类型，`intent/proposed_action` 必须在白名单内且组合自洽，例如 `find_players + ignore`、`irrelevant + create_game` 这类矛盾输出会被视为 contract failure；每个 slot 也必须是 `SlotValue` contract，包含 `value/source/confidence/confirmed/needs_confirmation`，且 source、confidence、布尔字段类型都要合法，`value` 不能为空或 `unknown`，`confirmed` 与 `needs_confirmation` 不能互相矛盾。模型缺少 `proposed_action`、输出半结构化 slot 或输出自相矛盾的意图/动作时，不再由后端根据 intent/默认值猜动作和槽位，而是记录 `llm_contract.accepted=false`、写入 `contract_errors` 并转人工，防止不完整模型输出继续进入状态机和工具链。
...
- `trial_store_rows.py` 已新增，负责试用台 SQLite row -> dict 的纯投影：客户、审批、trace、状态迁移事件和发送尝试。它不查询数据库、不推进状态、不做麻将业务判断；`scripts/run_boss_trial_app.py` 只调用这些投影函数，后续再拆仍需要 store lookup 的 game/outbox/followup 投影。
