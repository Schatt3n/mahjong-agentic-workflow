...
- `candidate_semantics.py` 已新增 `CandidateSemanticProposalAdapter` 和 `CandidateSemanticProposalResult`，把候选人回复的 LLM/fallback 语义提案收敛成单一 contract：模型只提出 `semantic_type/proposed_action/reply_text/confidence/reasoning_summary`，后端 validator 再决定是否允许写状态；候选人 `semantic_type/proposed_action/feedback_type` 的白名单和别名映射也已从脚本移入 contract 层。当前具体 LLM prompt 和候选人提案归一化仍在脚本内，followup 草稿器仍由脚本回调提供，后续应继续拆成独立模块。
