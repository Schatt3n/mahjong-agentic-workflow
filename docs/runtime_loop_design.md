...
微信通道当前通过 WeChaty 做白名单灰度。WeChaty bridge 只负责平台消息收发、原始消息 ID 和投递账本，业务状态仍由 Agent Runtime 与 Store 管理。
...
显式托管的微信群不会直接复用私聊入口，而是经过 `group_chat/GroupMessageHandler`：

```text
群消息
  -> L1 严格协议解析（报局 / 编号认领 / 忽略）
  -> L2 路由（群内短答 / 切私聊 / 主 Agent / 忽略）
  -> 必要时进入 L3 主 Agent
```

L1 只处理稳定、可验证的业务协议，例如 `14:00 0.5 无烟 371` 和 `2来`，宁可漏判也不把开放式语义写成不断扩张的关键词树。L2 只决定处理位置，不生成业务结论。L3 使用原有 AgentLoop，但群内回复注入长度和隐私约束。
...
因此同一个群的 A、B、C 各自拥有独立 recent turns 和 checkpoint。模型拿到的是房间公开看板投影，不是整段房间聊天记录。

### 14.2 群转私聊

`ChannelIdentity` 维护 WeChat 外部账号与稳定 `customer_id`、私聊 `conversation_id`、好友/可私聊能力的映射。复杂请求且用户可私聊时，后端：

1. 在群内发送最短确认。
2. 写入 10 分钟有效的 `ChannelSwitch`。
3. 构建只包含该用户原文、画像摘要、已解析需求和缺失字段的 `SystemTriggerMessage`。
4. 在目标私聊会话锁内运行同一个主 Agent。
5. 将经过客户可见审查的回复投递到私聊。

模型不选择发送通道，也不会看到房间其他用户的原始消息。若用户在 10 分钟内又把相关补充发到群里，系统只提醒其查看私聊，不把两条会话原文混在一起。

### 14.3 看板投影与编号

`BoardEngine` 从关联到房间的活跃 Game 生成公开看板。普通状态变化进入 30 秒合并窗口，座位认领、释放等关键变化会将持久化发布任务提前到当前时间。工具成功后由 `GroupBoardTrigger` Hook 调度刷新，因此 ToolGateway 不直接承担通道发送。

每次成功发送都保存不可变 `BoardSnapshot` 与 `BoardItem(item_no, game_id)`。用户引用某版看板时通过平台消息 ID 精确解析；没有引用时只读取最新版。旧看板编号不会被后端猜测性回退，以免同一个“2来”在重排后加入错误的局。

### 14.4 原子认领与通知

`ClaimHandler` 先解析看板版本，再检查局状态、重复参与和时间冲突，最后在同一个 SQLite `BEGIN IMMEDIATE` 事务中写认领记录与参与者状态。`source_conversation_id + source_message_id` 保证重复微信消息不会重复占位；最后一个座位的并发竞争最多一人成功。

好友优先私聊确认，非好友只在群内发送最小公开结果。所有通知只接收 `public_group_game_summary()` 允许列表投影，不包含微信备注、其他参与者身份、私聊原文或内部席位结构。

### 14.5 权限开关

只读观察群和托管群是两套独立配置。只有显式托管的群才能创建 Game、认领座位和排队更新看板；托管群自动外发仍由单独的 `outbound_enabled` 控制，默认关闭。
...
| `runtime_group_game_claims` | 群编号认领和来源消息幂等记录 |
...
当前 `GroupBoardTrigger` 已挂在 `after_tool_execute`：当主 Agent 成功创建或修改与托管群关联的 Game 时，它只创建持久化看板刷新任务，不直接在 ToolGateway 中发送微信消息。这证明主链路可以在不增加业务分支的前提下扩展领域投影；Hook 自身仍不得绕过事务、权限和外发开关。
