...
BOSS_TRIAL_GOLDEN_PATH = EVAL_DIR / "boss_trial_golden.jsonl"
BADCASE_PATH = EVAL_DIR / "badcases.jsonl"
...
如果 search_current_open_games 已调用但 result_count=0，说明当前没有匹配的现成/快开局；回复“暂时没有，要不要帮你组一个”即可，不要继续追问时间、档位、烟况。
...
{"reply_text":"一句可复制给当前客户的回复","selected_pool_game_id":"可选，只能来自工具返回的 game_id","risk_level":"low|medium|high","reasoning_summary":"一句话说明为什么这样回复","notes":["简短说明"]}"""
        prompt = {
            ...
            "rules": [
                ...
                "如果工具 search_current_open_games 已调用但没有候选局，先告知当前没有对应局，再问要不要新组。",
