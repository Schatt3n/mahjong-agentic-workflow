...
class BossTrialService:
    ...
    def analyze_controlled(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        persistence = self._persist_controlled_workflow_result(
    ...
    def _persist_controlled_workflow_result(
        self,
        *,
        workflow_result: Any,
        ...
        if not validated.allowed:
            return {"persisted": False, "reason": "validated_action_not_allowed", "code": validated.code}
        if validated.effective_action.value != "queue_invites":
            return {"persisted": False, "reason": "no_state_write_for_action", "effective_action": validated.effective_action.value}
        ...
        parsed = self._controlled_parsed_for_store(
            projected=projected,
            game_id=game_id,
            sender_id=sender_id,
            sender_name=sender_name,
        ...
        create_result = self._execute_controlled_action(
            create_game_action,
            lambda: self._create_controlled_game_state_write(
                game_id=game_id,
                status=status,
                organizer_id=sender_id,
                organizer_name=sender_name,
        ...
        if outbox_tool and outbox_tool.called and outbox_tool.allowed and projected_outbox:
            outbox_action = self._workflow_state_action_record(
                trace_id=trace_id,
            ...
            outbox_result = self._execute_controlled_action(
                outbox_action,
                lambda: self._create_controlled_outbox_state_write(
        ...
        persisted_game = self._game_by_id(game_id)
        agent_actions = [
    ...
    def _create_controlled_game_state_write(
        self,
        *,
        game_id: str,
        status: str,
    ...
    def _create_controlled_outbox_state_write(
        self,
        *,
        action: dict[str, Any],
        game_id: str,
    ...
    def _controlled_parsed_for_store(
        self,
        *,
        projected: dict[str, Any],
        game_id: str,
        ...
        rules = self._controlled_rules_for_store(parsed, game_label)
        start_time = self._controlled_start_time_for_store(parsed)
        ...
        return {
            **parsed,
            ...
            "summary": str(parsed.get("summary") or "").strip() or " ".join(
                str(part)
                for part in [
                    game_label,
    ...
    def _controlled_rules_for_store(self, parsed: dict[str, Any], game_label: str) -> list[str]:
    ...
    def _controlled_start_time_for_store(self, parsed: dict[str, Any]) -> str:
        start_time = str(parsed.get("start_time") or "").strip()
    ...
    def _fallback_candidate_action_proposal(
        self,
        text: str,
        outbox_item: dict[str, Any],
        game: dict[str, Any] | None,
    ) -> dict[str, Any]:
        classification = self._classify_candidate_reply(text, game)
        feedback_type = str(classification.get("feedback_type") or "candidate_question")
        return {
            "source": "rules",
            "model": None,
    ...
    def _llm_candidate_action_proposal(
        self,
        *,
        trace_id: str,
        candidate_text: str,
        ...
        system_prompt = """你是麻将馆候选人回复的语义解析器和动作提案器。
...
{"semantic_type":"accepted|arrived|declined|ask_later|candidate_question|candidate_negotiation|do_not_disturb|uncertain","proposed_action":"动作名","confidence":0.0,"reply_text":"老板建议回复","risk_level":"low|medium|high","reasoning_summary":"一句话原因","extracted_facts":{"requested_start_time":"可选 HH:MM","requested_start_at":"可选 ISO 时间","requested_duration_hours":null},"notes":["可选简短说明"]}"""
        context = self._candidate_action_context(
            candidate_text=candidate_text,
        ...
        if not budget_decision.allowed:
            ...
            return {**fallback, "model": self.llm_config.model, "reasoning_summary": "LLM 预算拒绝，使用后端安全降级语义判断。"}
        ...
        try:
            ...
            return {**fallback, "model": self.llm_config.model, "reasoning_summary": f"LLM 语义提案失败，使用安全降级：HTTP {exc.code}"}
            ...
            return {**fallback, "model": self.llm_config.model, "reasoning_summary": f"LLM 语义提案失败，使用安全降级：{type(exc).__name__}"}
        ...
        parsed = self._parse_llm_reply_json(
            ...
            trace_id=trace_id,
            stage="candidate_action_proposal",
            schema_hint='{"semantic_type":str,"proposed_action":str,"confidence":number,"reply_text":str,"risk_level":str,"reasoning_summary":str}',
        )
        proposal = self._normalize_candidate_action_proposal(
            parsed,
            fallback=fallback,
            source="llm",
            model=self.llm_config.model,
    ...
    def _normalize_candidate_action_proposal(
        self,
        parsed: dict[str, Any],
        *,
        fallback: dict[str, Any],
        ...
        if confidence is None:
            confidence = self._safe_float(fallback.get("confidence")) or 0.0
    ...
    def _apply_llm_extracted_negotiation_facts(
        ...
        duration = self._safe_float(facts.get("requested_duration_hours"))
        ...
        if requested_start is not None and "requested_start_time" not in classification:
            ...
            classification["requested_start_time_label"] = self._natural_time_label(requested_start)
            if game_start is not None:
                classification["current_start_at"] = game_start.isoformat()
                classification["current_start_time"] = game_start.strftime("%H:%M")

    def _candidate_contract_negotiation_classification(
        self,
        text: str,
        game: dict[str, Any] | None,
    ) -> dict[str, Any] | None:
        start_request = self._candidate_requested_start_time(text, game)
        game_start = self._game_start_at(game)
        if start_request is not None and game_start is not None and abs((start_request - game_start).total_seconds()) >= 15 * 60:
            return {
                ...
                "requested_start_time_label": self._natural_time_label(start_request),
                "current_start_at": game_start.isoformat(),
                "current_start_time": game_start.strftime("%H:%M"),
            }
        duration_request = self._candidate_requested_duration(text)
        game_duration = self._game_duration_hours(game)
    ...
    def _classify_candidate_reply(self, text: str, game: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        contract_negotiation = self._candidate_contract_negotiation_classification(text, game)
    ...
    def _game_duration_hours(self, game: dict[str, Any] | None) -> float | None:
        parsed = game.get("parsed") if isinstance(game, dict) and isinstance(game.get("parsed"), dict) else {}
        return self._safe_float(parsed.get("duration_hours"))

    def _game_start_at(self, game: dict[str, Any] | None) -> datetime | None:
        parsed = game.get("parsed") if isinstance(game, dict) and isinstance(game.get("parsed"), dict) else {}
        return parse_dt(parsed.get("start_at"))

    def _candidate_requested_start_time(self, text: str, game: dict[str, Any] | None) -> datetime | None:
        game_start = self._game_start_at(game)
        ...
        if hour is None:
            ...
            if match:
                hour = self._chinese_hour(match.group(1))
                suffix = match.group(2) or ""
                minute = 30 if "半" in suffix else self._minute_from_chinese_or_digits(suffix)
        if hour is None:
            return None
    ...
    def _chinese_hour(self, value: str) -> int | None:
        digits = {"一": 1, "二": 2, "两": 2, "三": 3, "四": 4, "五": 5, "六": 6, "七": 7, "八": 8, "九": 9, "十": 10}
        return digits.get(value)

    def _minute_from_chinese_or_digits(self, value: str) -> int:
        if not value:
            return 0
    ...
    def _natural_time_label(self, value: datetime) -> str:
        hour = value.hour
    ...
    def _candidate_requested_duration(self, text: str) -> float | None:
        normalized = re.sub(r"\s+", "", text.lower())
    ...
    def _organizer_negotiation_fallback(
        self,
        *,
        classification: dict[str, Any],
        candidate_name: str,
    ...
    def _guard_organizer_followup_text(
        self,
        text: str,
        *,
        fallback: str,
        ...
        cleaned = truncate_text(text.strip(), 300)
        if not cleaned:
            return fallback
        ...
        if requested_start and str(requested_start) not in cleaned:
            return fallback
        if requested_duration:
            duration_text = f"{int(requested_duration)}小时" if float(requested_duration).is_integer() else f"{float(requested_duration):g}小时"
    ...
    def _llm_organizer_followup_draft(
        self,
        *,
        trace_id: str,
        classification: dict[str, Any],
        ...
        system_prompt = """你是麻将馆老板的协商消息起草助手。
...
{"should_create_message":true,"message_text":"发给发起人的一句话","risk_level":"low|medium|high","reasoning_summary":"一句话原因","notes":["可选简短说明"]}"""
        ...
        parsed = self._parse_llm_reply_json(
            ...
            trace_id=trace_id,
            stage="organizer_followup_draft",
            schema_hint='{"should_create_message":bool,"message_text":str,"risk_level":str,"reasoning_summary":str}',
        )
        result = {
...
{"tool_calls":[{"tool_name":"search_current_open_games|search_candidate_customers|send_message","arguments":{},"reason":"一句原因"}],"reasoning_summary":"一句话"}"""
    ...
    def _validate_tool_action(
        ...
        *,
        ...
        spec = tool_spec_for_stage(tool_name, stage)
        available = {str(item.get("name")) for item in self._tool_specs_for_stage(stage)}
        critical_missing = sorted(set(missing_fields) & CRITICAL_FIELDS)
        notes: list[str] = []
        ...
        runtime_policy = self.store.runtime_policy()
        if (
            bool(proposal.get("side_effect"))
            and runtime_policy.get("llm_required_for_side_effect_tools")
        ):
            proposer = str(proposal.get("proposed_by") or proposal.get("source") or "").strip()
            source = str(proposal.get("source") or "").strip()
            if not trusted_action_proposer(proposer, source):
                return {
                    "allowed": False,
        ...
        stripped_arguments = sorted(
            key for key in dict(args)
            if key not in allowed_arg_names and key != "requested_execution_mode"
        )
