...
- 麻将馆短码是组局槽位，不是玩法名：`173=一缺三`，表示 `known_player_count=1, needed_seats=3`；`272=二缺二`；`371=三缺一`；`216` 这类数字如果出现在川麻语境里通常表示档位/封顶，不要和人数短码混淆。
...
- 一个联系人可能代表多个座位。用户说“我这边两个人/我们 3 个/带一个朋友”时，后续记录候选人确认或建局参与者必须用 `seat_count` 保存真实座位数；不要把一个微信联系人固定当成 1 人。
